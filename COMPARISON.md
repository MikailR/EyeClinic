# Comparison notes

One to three sentences per direction: Trinidad mobile patients, CTA clarity, clutter risk.

## grok-4.6 — PASS
Storefront-faithful sparse blue/white with both surgeons and the full diagnostic/advanced lists behind a flagship strip. Sticky Call + Book plus header WhatsApp keep phone-first Trinidad booking obvious; risk is a slightly longer scroll once “see all” opens, but the resting home stays calm.

## fable-5.1 — PASS
Clean clinic-blue system, six flagships then a disclosure for the storefront lists, and a WhatsApp-bound book form. Thumb-sized sticky bar and visible phone text read well on mid-range Android; clutter risk is low unless every secondary CTA fights the sticky bar on small screens.

## opus-5 — PASS
Lightest gzip footprint of the set, phone number repeated as plain text, and booking that only opens `wa.me` — strong local trust pattern. Sparse royal-blue layout avoids card walls; CTA hierarchy (one primary above the fold, sticky Call/Book) is the clearest of the four shipped so far.

## gpt-6-astra — PENDING
Codex still writing after repeated code-mode stalls; placeholder iframe until `dist/` lands. Will re-critique when the preview ships.

## claude-design — PASS
Design-MCP explored three mobile canvases then merged storefront-blue hero + doctor cards + thin-rule service lists. Strong five-second glance and verified 44px targets; the deep blue band builds door-panel recognition without overcrowding the resting home.
